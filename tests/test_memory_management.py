...
async def setup_workflow(test_broker, test_topic, reset_tracking):
    ...
    async def processor_agent(message):
        """Process sensor data"""
        ...
        sensor_data = message.get("dependency_messages", {}).get("sensor", {}).get("data", {})
    ...
    @stella_agent(name="analyzer", broker=test_broker, topic=test_topic, depends_on=["processor"])
    async def analyzer_agent(message):
        ...
        # Analyze the data
        analyzer_output = {
            "analysis_result": "normal" if processor_data.get("processed_value") < 100 else "high",
        ...
        logger.info(f"Analyzer agent called, returning: {analyzer_output}")
        return analyzer_output
    
    yield

async def test_state_memory_updates(setup_workflow, test_broker, test_topic):
    ...
    assert sensor_state["messages_processed"] == 1, "Sensor messages_processed count incorrect"
    ...
    assert "messages_processed" in processor_state, "Processor state missing messages_processed"
    assert processor_state["messages_processed"] == 1, "Processor messages_processed count incorrect"
    assert "last_processed_time" in processor_state, "Processor state missing last_processed_time"
    ...
    assert "messages_processed" in analyzer_state, "Analyzer state missing messages_processed"
    assert analyzer_state["messages_processed"] == 1, "Analyzer messages_processed count incorrect"
    assert "last_processed_time" in analyzer_state, "Analyzer state missing last_processed_time"
    ...
    assert processor_output.get("processed_value") == 84, "Processor did not transform the data correctly"
    
    assert analyzer_output is not None, "Analyzer did not generate output"
    assert analyzer_output.get("analysis_result") == "normal", "Analyzer did not analyze the data correctly"
    
async def test_dependency_memory_management(setup_workflow, test_broker, test_topic):
    ...
    assert processor_output is not None, "Processor did not generate output"
    assert processor_output.get("original_timestamp") == sensor_output.get("timestamp"), \
        "Processor did not receive sensor's timestamp"
    
    # Verify that the analyzer received the processor's output in its dependencies
    assert analyzer_output is not None, "Analyzer did not generate output"
    assert analyzer_output.get("processed_value") == processor_output.get("processed_value"), \
        "Analyzer did not receive processor's value"
    
    # Verify dependency memory structure
    # This would require accessing the broker's memory directly, which might not be possible in the test
