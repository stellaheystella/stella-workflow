...
async def test_simple_message_flow(setup_workflow, test_broker, test_topic):
    ...
    assert processor_output.get("value") == "RAW_DATA", "Processor did not transform the data correctly"
...
async def test_dependency_resolution(setup_workflow, test_broker, test_topic):
    ...
    assert consumer_output.get("processed_timestamp") == processor_output.get("processed_timestamp"), \
