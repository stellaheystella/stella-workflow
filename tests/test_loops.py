...
from stella_workflow import in_loop, stella_agent
...
async def setup_workflow(
    ...
    async def user_list_agent() -> dict[str, Any]:
    ...
    async def email_sender_agent(message: dict[str, Any]) -> dict[str, Any] | None:
    ...
    async def email_logger_agent(message: dict[str, Any]) -> dict[str, Any]:
...
async def test_loop_execution(
    ...
    assert email_sender_called == 3, (
        f"Email sender was not called for each user (called {email_sender_called} times)"
    )
    ...
    assert email_logger_called == 3, (
        f"Email logger was not called for each email (called {email_logger_called} times)"
    )
    ...
    for i, output in enumerate(email_sender_outputs):
        assert output.get("email") == user_list_output.get("users")[i].get("email"), (
            f"Email sender output {i} has incorrect email"
        )
    ...
    for i, output in enumerate(email_logger_outputs):
        assert output.get("logged_email") == email_sender_outputs[i].get("email"), (
            f"Email logger output {i} has incorrect email"
        )
